...
@php $pageTitle = 'Edit Blok'; @endphp
...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="bg-white p-6 rounded-xl border border-surface-border shadow-sm">
            <h2 class="text-lg font-bold text-text-main mb-6">Form Edit Blok</h2>
            
            <form action="{{ route('admin.blocks.update', $block) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="division_id" id="division_id" required
                        ...
                        @foreach($divisions as $division)
                            <option value="{{ $division->id }}" {{ old('division_id', $block->division_id) == $division->id ? 'selected' : '' }}>{{ $division->name }}</option>
                        @endforeach
                    </select>
                    @error('division_id')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="area_hectares" id="area_hectares" step="0.01" min="0"
                               value="{{ old('area_hectares', $block->area_hectares) }}"
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="inactive" {{ old('status', $block->status) === 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full px-3 py-2 rounded-lg border border-surface-border focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm">{{ old('description', $block->description) }}</textarea>
                </div>
