...
@section('title', 'Input BAP Material')

@php $pageTitle = 'BAP Material'; @endphp

@section('content')
    <form action="{{ route('admin.bap-material.store') }}" method="POST" enctype="multipart/form-data" id="bapMaterialForm">
        @csrf
        
        <div class="bg-white p-6 rounded-xl border border-surface-border shadow-sm mb-6">
            <h3 class="text-lg font-semibold text-text-main mb-4">Input Data Material</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" name="jenis_material" value="{{ old('jenis_material') }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" name="panjang" value="{{ old('panjang') }}" step="0.01" min="0"
                           placeholder="0.00"
                ...
                <div>
                    ...
                    <input type="number" name="lebar" value="{{ old('lebar') }}" step="0.01" min="0"
                           placeholder="0.00"
                           class="w-full h-10 px-3 rounded-lg border border-surface-border text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="tinggi" value="{{ old('tinggi') }}" step="0.01" min="0"
                           placeholder="0.00"
                           class="w-full h-10 px-3 rounded-lg border border-surface-border text-sm">
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="keterangan" rows="3" 
                          ...
                          class="w-full px-3 py-2 rounded-lg border border-surface-border text-sm resize-none">{{ old('keterangan') }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl border border-surface-border shadow-sm mb-6">
            ...
            <div class="mb-4">
                <label class="block text-xs font-medium text-text-secondary mb-2">UPLOAD FOTO DOKUMENTASI (BISA LEBIH DARI 1)</label>
                <div class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-green-300 bg-green-50 rounded-lg cursor-pointer hover:bg-green-100 transition-colors" 
                     onclick="document.getElementById('photoDokumentasiInput').click()">
                    <span class="material-symbols-outlined text-[36px] text-primary mb-2">add_photo_alternate</span>
                    <p class="text-sm text-text-main font-medium">Klik untuk upload foto dokumentasi</p>
                    <p class="text-xs text-text-secondary/70">JPG, PNG (Max 5MB per foto)</p>
                </div>
        ...
        <div class="bg-white p-6 rounded-xl border border-surface-border shadow-sm mb-6">
            ...
            <div class="mb-4">
                <label class="block text-xs font-medium text-text-secondary mb-2">UPLOAD FOTO SURAT JALAN (BISA LEBIH DARI 1)</label>
                <div class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-blue-300 bg-blue-50 rounded-lg cursor-pointer hover:bg-blue-100 transition-colors" 
                     onclick="document.getElementById('photoSuratJalanInput').click()">
                    <span class="material-symbols-outlined text-[36px] text-blue-500 mb-2">upload_file</span>
                    <p class="text-sm text-text-main font-medium">Klik untuk upload foto surat jalan</p>
                    <p class="text-xs text-text-secondary/70">JPG, PNG (Max 5MB per foto)</p>
                </div>
