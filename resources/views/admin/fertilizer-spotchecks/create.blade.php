...
@section('title', 'Input Spotchek Pemupukan')

@php $pageTitle = 'Form Spotchek Pemupukan'; @endphp
...
@section('content')
    <div class="max-w-4xl">
        ...
        <div class="bg-white p-6 rounded-xl border border-surface-border shadow-sm">
            <h2 class="text-lg font-bold text-text-main mb-6">Form Input Spotchek Pemupukan</h2>
            ...
            <form action="{{ route('admin.fertilizer-spotchecks.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <label class="flex items-center gap-2 px-3 py-2 rounded-lg border border-surface-border bg-[#edf3e7] cursor-pointer">
                                    <input type="radio" name="status" value="pending" {{ old('status') == 'pending' || !old('status') ? 'checked' : '' }} class="text-primary focus:ring-primary">
                                    <span class="text-sm">Pending</span>
                                </label>
                                <label class="flex items-center gap-2 px-3 py-2 rounded-lg border border-surface-border bg-[#edf3e7] cursor-pointer">
                                    <input type="radio" name="status" value="completed" {{ old('status') == 'completed' ? 'checked' : '' }} class="text-primary focus:ring-primary">
                    ...
                    <div class="space-y-4">
                            ...
                            <select name="fertilizer_id" id="fertilizer_id" required
                                ...
                                @foreach($fertilizers as $fertilizer)
                                    <option value="{{ $fertilizer->id }}" 
                                            ...
                                            {{ old('fertilizer_id') == $fertilizer->id ? 'selected' : '' }}>
                        ...
                        <div class="p-4 bg-red-50 rounded-lg border border-red-200">
                            ...
                            <div class="text-xl font-bold text-red-600" id="penalty_amount_display">Rp 0</div>
                ...
                <div class="mt-4">
                    ...
                    <textarea name="findings" id="findings" rows="3"
                           class="w-full px-3 py-2 rounded-lg border border-surface-border focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm bg-[#edf3e7] resize-none">{{ old('findings') }}</textarea>
                </div>
