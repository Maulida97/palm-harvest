...
@section('title', 'Edit Internal Memo ' . ucfirst($type))

@php $pageTitle = 'Edit Internal Memo ' . ucfirst($type); @endphp
...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="bg-white p-6 rounded-xl border border-surface-border shadow-sm">
            <h2 class="text-lg font-bold text-text-main mb-6">Form Edit Internal Memo {{ ucfirst($type) }}</h2>
            ...
            <form action="{{ route('admin.memo.update', [$type, $memo]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="no_item" id="no_item" required
                               value="{{ old('no_item', $memo->no_item) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="berlaku" id="berlaku" required
                                   value="{{ old('berlaku', $memo->berlaku->format('Y-m-d')) }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="tidak_berlaku" id="tidak_berlaku"
                                   value="{{ old('tidak_berlaku', $memo->tidak_berlaku ? $memo->tidak_berlaku->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_revisi" id="tanggal_revisi"
                               value="{{ old('tanggal_revisi', $memo->tanggal_revisi ? $memo->tanggal_revisi->format('Y-m-d') : '') }}"
                               class="w-full h-10 px-3 rounded-lg border border-surface-border focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm bg-[#edf3e7]">
                    </div>
                    ...
                    @if($memo->file_path)
                        <div>
                            <label class="block text-sm font-medium text-text-main mb-1">File Saat Ini</label>
                            <div class="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
                                ...
                                <a href="{{ asset('storage/' . $memo->file_path) }}" target="_blank" class="text-green-700 text-sm flex-1 hover:underline">
                    ...
                    <!-- Upload File PDF -->
                    <div>
                        <label for="file" class="block text-sm font-medium text-text-main mb-1">Ganti File PDF <span class="text-text-secondary text-xs">(Opsional - kosongkan jika tidak ingin mengganti)</span></label>
                        <div id="dropZone" class="border-2 border-dashed border-surface-border rounded-lg bg-[#edf3e7] p-6 flex flex-col items-center justify-center cursor-pointer hover:border-primary/50 transition-colors" onclick="document.getElementById('file').click()">
                            ...
                            <p class="text-text-secondary text-sm text-center">Klik untuk upload file PDF baru</p>
                            <p class="text-text-secondary text-xs mt-1">Maksimal 5MB</p>
                            <input type="file" name="file" id="file" accept=".pdf" class="hidden" onchange="showFileName(this)">
                        </div>
