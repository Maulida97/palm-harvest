...
@section('title', 'Edit Data QC')

@php $pageTitle = 'Edit Data QC'; @endphp
...
@section('content')
    <div class="max-w-4xl">
        ...
        <div class="bg-white p-6 rounded-xl border border-surface-border shadow-sm">
            <h2 class="text-lg font-bold text-text-main mb-6">Edit Data QC #{{ $bap->id }}</h2>
            ...
            <form action="{{ route('admin.bap.update', $bap) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="status" id="status" value="{{ $bap->verification_status }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <select name="division_id" id="division_id" required
                                    ...
                                    @foreach($divisions as $division)
                                        <option value="{{ $division->id }}" data-blocks='@json($division->blocks)' {{ old('division_id', $bap->block->division_id ?? '') == $division->id ? 'selected' : '' }}>{{ $division->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="end_date" id="end_date"
                                       value="{{ old('end_date', $bap->harvest_date?->format('Y-m-d') ?? date('Y-m-d')) }}"
                                       class="w-full h-10 px-3 rounded-lg border border-surface-border focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm bg-[#edf3e7]">
                            </div>
                            <div>
                                ...
                                <input type="number" name="luas" id="luas" step="0.01" min="0" readonly
                                       value="{{ old('luas', $bap->block->area_hectares ?? '') }}"
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="tree_count" id="tree_count" readonly
                                       value="{{ old('tree_count', $bap->block->tree_count ?? '') }}"
                                       placeholder="Otomatis dari master data"
                                       class="w-full h-10 px-3 rounded-lg border border-surface-border focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm bg-gray-100 cursor-not-allowed">
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="2"
                                   class="w-full px-3 py-2 rounded-lg border border-surface-border focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm bg-[#edf3e7] resize-none">{{ old('notes', $bap->notes) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="harvest_date" id="harvest_date"
                                       value="{{ old('harvest_date', $bap->harvest_date?->format('Y-m-d')) }}"
                            ...
                            <div>
                                ...
                                <input type="date" name="start_date" id="start_date"
                                       value="{{ old('start_date', $bap->created_at->format('Y-m-d')) }}"
                                       class="w-full h-10 px-3 rounded-lg border border-surface-border focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm bg-[#edf3e7]">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            @if($bap->image)
                                <div class="mb-2 border-2 border-surface-border rounded-lg bg-[#edf3e7] p-2 overflow-hidden">
                                    <img src="{{ asset('storage/' . $bap->image) }}" alt="Gambar saat ini" class="w-full h-[120px] object-cover rounded-lg">
                                    <p class="text-xs text-text-secondary text-center mt-1">Gambar saat ini (klik di bawah untuk ganti)</p>
                                </div>
                            @endif
                            <div class="border-2 border-dashed border-surface-border rounded-lg bg-[#edf3e7] p-4 {{ $bap->image ? 'h-[80px]' : 'h-[180px]' }} flex flex-col items-center justify-center cursor-pointer hover:border-primary/50 transition-colors" onclick="document.getElementById('image').click()">
                                <span class="material-symbols-outlined text-text-secondary text-4xl mb-2">add_photo_alternate</span>
                                <p class="text-text-secondary text-sm">{{ $bap->image ? 'Ganti gambar' : 'Klik untuk upload gambar' }}</p>
                        ...
                        <div class="p-4 bg-surface-light rounded-lg border border-surface-border">
                            <h3 class="text-sm font-semibold text-text-main mb-3">Informasi Data</h3>
    ...
    <script>
        // Store blocks data
        let blocksData = {};
        const currentBlockId = {{ $bap->block_id }};
        const currentDivisionId = {{ $bap->block->division_id ?? 'null' }};
        ...
        function updateBlocks(initBlockId = null) {
            ...
            if (divisionId && blocksData[divisionId]) {
                ...
                // If block was selected, update info
                if (initBlockId) {
                    updateBlockInfo();
                }
            }
